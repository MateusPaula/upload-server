...
// We don't need to use the ZOD schema here because there is no serialization happening and 
// we don't need to validate the input, like the parse for `getUploadsInput`
...
export async function getUploads(
    ...
    // Get entered page by user minus 1 * 20 (this will skip the first 20 results)    
    const [uploads, [{ total }]] = await Promise.all([
        db
          ...
          .orderBy(fields => {
            // We can use the id since we're using uuidv7 that is sortable
